var paper = new function(undefined) {
	var Base = new function() {
		...
		function each(obj, iter, bind) {
			if (obj)
				('length' in obj && !obj.getLength && typeof obj.length === 'number' ? forEach : forIn).call(
					obj,
					iter,
					(bind = bind || obj)
				);
		...
		statics: {
			...
			readAll: function(list, start, options) {
				...
				for (var i = start || 0, l = list.length; i < l; i++) {
					res.push(
						Array.isArray((entry = list[i])) ? this.read(entry, 0, options) : this.read(list, i, options, 1)
					);
				}
				return res;
			},
	...
	var PaperScope = Base.extend({
		...
		initialize: function PaperScope() {
			...
			if (!this.browser) {
				...
				if (platform) browser[platform] = true;
				agent.replace(
	...
	var Rectangle = Base.extend(
		{
			...
			setRight: function(right) {
				...
				if (this._fixW) this.x = right - this.width;
				else this.width = right - this.x;
				this._fixX = 1;
			},
			...
			setBottom: function(bottom) {
				...
				if (this._fixH) this.y = bottom - this.height;
				else this.height = bottom - this.y;
				this._fixY = 1;
			},
	...
	var Matrix = Base.extend(
		{
			...
			toString: function() {
				...
				return (
					'[[' +
					[ f.number(this._a), f.number(this._b), f.number(this._tx) ].join(', ') +
					'], [' +
					[ f.number(this._c), f.number(this._d), f.number(this._ty) ].join(', ') +
					']]'
				);
			},
			...
			isIdentity: function() {
				return (
					this._a === 1 && this._c === 0 && this._b === 0 && this._d === 1 && this._tx === 0 && this._ty === 0
				);
			},
	...
	var Project = PaperScopeItem.extend({
		...
		insertChild: function(index, item, _preserve) {
			if (item instanceof Layer) {
				...
				(this._activeLayer || this.insertChild(index, new Layer(Item.NO_INSERT)))
	...
	var Item = Base.extend(
		Emitter,
		{
			...
			_initialize: function(props, point) {
				...
				if (hasProps && props !== Item.NO_INSERT)
					this._set(props, { insert: true, project: true, parent: true }, true);
				return hasProps;
			},
		...
		Base.each(
			[ 'bounds', 'strokeBounds', 'handleBounds', 'roughBounds', 'internalBounds', 'internalRoughBounds' ],
			function(key) {
				...
				this[getter] = function(_matrix) {
					var boundsGetter = this._boundsGetter,
						name =
							(!internalGetter &&
								(typeof boundsGetter === 'string'
									? boundsGetter
									: boundsGetter && boundsGetter[getter])) ||
							getter,
						bounds = this._getCachedBounds(name, _matrix, this, internalGetter);
					return key === 'bounds'
						? new LinkedRectangle(bounds.x, bounds.y, bounds.width, bounds.height, this, 'setBounds')
						: bounds;
				};
			},
			{
		...
		},
		{
			...
			intersects: function(item, _matrix) {
				...
				return (
					this._asPathItem().getIntersections(item._asPathItem(), null, _matrix || item._matrix, true)
						.length > 0
				);
			},
			...
			_hitTest: function(point, options) {
				...
				var matrix = this._matrix,
					...
					).multiply(Math.max(options.tolerance, 1e-6)));
				point = matrix._inverseTransform(point);

				if (
					!this._children &&
					!this.getInternalRoughBounds().expand(tolerancePadding.multiply(2))._containsPoint(point)
				)
					return null;
				var checkSelf = !(
			...
			matches: function(name, compare) {
				...
				if (type === 'object') {
					...
					var value = /^(empty|editable)$/.test(name)
						...
						: name === 'type' ? Base.hyphenate(this._class) : this[name];
			...
			statics: {
				_getItems: function _getItems(children, match, matrix, param, firstOnly) {
					if (!param && typeof match === 'object') {
						...
						if (bounds) match = Base.set({}, match, { inside: true, overlapping: true });
					}
					...
					for (var i = 0, l = children && children.length; i < l; i++) {
						...
						if (rect) {
							...
							if (
								!(param.inside && rect.contains(bounds)) &&
								!(
									param.overlapping &&
									(bounds.contains(rect) || param.path.intersects(child, childMatrix))
								)
							)
								add = false;
						}
		...
		},
		{
			...
			rotate: function(angle) {
				return this.transform(
					new Matrix().rotate(angle, Point.read(arguments, 1, { readNull: true }) || this.getPosition(true))
				);
			}
		},
		Base.each(
		...
		),
		{
			transform: function(matrix, _applyMatrix, _applyRecursively, _setApplyMatrix) {
				...
				var _matrix = this._matrix,
					applyMatrix =
						(_applyMatrix || this._applyMatrix) &&
						(!_matrix.isIdentity() || matrix || (_applyMatrix && _applyRecursively && this._children));
				if (!matrix && !applyMatrix) return this;
				if (matrix) _matrix.preConcatenate(matrix);
				if (
					(applyMatrix = applyMatrix && this._transformContent(_matrix, _applyRecursively, _setApplyMatrix))
				) {
					var pivot = this._pivot,
						style = this._style,
	...
	var Shape = Item.extend(
		{
			...
			toPath: function(insert) {
				var path = this._clone(
					new Path[(Base.capitalize(this._type))]({
						center: new Point(),
						size: this._size,
						radius: this._radius,
						insert: false
			...
			return {
				_contains: function _contains(point) {
					if (this._type === 'rectangle') {
						...
						return center
							? point.subtract(center).divide(this._radius).getLength() <= 1
	...
	var Raster = Item.extend(
		{
			...
			setCrossOrigin: function(crossOrigin) {
				...
				if (this._image) this._image.crossOrigin = crossOrigin;
			},
	...
	var Segment = Base.extend({
		...
		_serialize: function(options) {
			return Base.serialize(
				this.hasHandles() ? [ this._point, this._handleIn, this._handleOut ] : this._point,
				options,
				true
			);
		},
		...
		isSelected: function(_point) {
			...
			return !_point
				...
				: _point === this._point
					? !!(state & 4)
					: _point === this._handleIn ? !!(state & 1) : _point === this._handleOut ? !!(state & 2) : false;
		},
		...
		setSelected: function(selected, _point) {
			var path = this._path,
				...
				flag = !_point
					? 7
					: _point === this._point ? 4 : _point === this._handleIn ? 1 : _point === this._handleOut ? 2 : 0;
			if (selected) {
				state |= flag;
			} else {
				state &= ~flag;
		...
		getPrevious: function() {
			var segments = this._path && this._path._segments;
			return (
				(segments && (segments[this._index - 1] || (this._path._closed && segments[segments.length - 1]))) ||
				null
			);
		},
	...
	var SegmentPoint = Point.extend({
		...
		_serialize: function(options) {
			var f = options.formatter,
				x = f.number(this._x),
				y = f.number(this._y);
			return this.isSelected() ? { x: x, y: y, selected: true } : [ x, y ];
		},
	...
	var Curve = Base.extend(
		{
			...
			getPrevious: function() {
				...
				return (
					(curves &&
						(curves[this._segment1._index - 1] || (this._path._closed && curves[curves.length - 1]))) ||
					null
				);
			},
			...
			getIntersections: function(curve) {
				return Curve._getIntersections(
					this.getValues(),
			...
			_getParameter: function(offset, isParameter) {
				return isParameter
					...
					: offset && offset.curve === this
						? offset.parameter
						: offset === undefined && isParameter === undefined ? 0.5 : this.getParameterAt(offset, 0);
			},
			...
			split: function(offset, isParameter) {
				return this._path
					? this._path.split(this._segment1._index, this._getParameter(offset, isParameter))
					: null;
			},
			...
			statics: {
				...
				getParameterOf: function(v, point) {
					...
					for (var c = 0; c < 2; c++) {
						...
						for (var i = 0; i < count; i++) {
							t = roots[i];
							if (point.isClose(Curve.getPoint(v, t), geomEpsilon)) return t;
						}
					}
				...
				getNearestParameter: function(v, point) {
					if (Curve.isStraight(v)) {
						...
						return u < 1e-12
							? 0
							: u > 0.999999999999 ? 1 : Curve.getParameterOf(v, new Point(p1x + u * vx, p1y + u * vy));
					}
		...
		Base.each(
			...
			},
			{
				...
				isCollinear: function(curve) {
					return (
						curve && this.isStraight() && curve.isStraight() && this.getLine().isCollinear(curve.getLine())
					);
				},
		...
		new function() {
			function addLocation(locations, param, v1, c1, t1, p1, v2, c2, t2, p2, overlap) {
				var startConnected = param.startConnected,
					...
					tMin = 4e-7,
					tMax = 1 - tMin;
				if (t1 == null) t1 = Curve.getParameterOf(v1, p1);
				if (t1 !== null && t1 >= (startConnected ? tMin : 0) && t1 <= (endConnected ? tMax : 1)) {
					if (t2 == null) t2 = Curve.getParameterOf(v2, p2);
			...
			return {
				statics: {
					_getIntersections: function(v1, v2, c1, c2, locations, param) {
						...
						if (
							!(
								max(c1p1x, c1s1x, c1s2x, c1p2x) >= min(c2p1x, c2s1x, c2s2x, c2p2x) &&
								min(c1p1x, c1s1x, c1s2x, c1p2x) <= max(c2p1x, c2s1x, c2s2x, c2p2x) &&
								max(c1p1y, c1s1y, c1s2y, c1p2y) >= min(c2p1y, c2s1y, c2s2y, c2p2y) &&
								min(c1p1y, c1s1y, c1s2y, c1p2y) <= max(c2p1y, c2s1y, c2s2y, c2p2y)
							)
						)
							return locations;
						...
						(straight
							? addLineIntersection
							: straight1 || straight2 ? addCurveLineIntersections : addCurveIntersections)(
							v1,
							v2,
							c1,
							c2,
						...
						if (straight && locations.length > before) return locations;
					...
					_getSelfIntersection: function(v1, c1, locations, param) {
						...
						if (side1 === side2) {
							...
							if (edgeSum * side1 > 0) return locations;
						}
					...
					getOverlaps: function(v1, v2) {
						...
						for (var i = 0, t1 = 0; i < 2 && pairs.length < 2; i += t1 === 0 ? 0 : 1, t1 = t1 ^ 1) {
							var t2 = Curve.getParameterOf(
								v[i ^ 1],
								new Point(v[i][t1 === 0 ? 0 : 6], v[i][t1 === 0 ? 1 : 7])
							);
							if (t2 != null) {
	...
	var CurveLocation = Base.extend(
		{
			...
			getSegment: function() {
				...
				if (!segment) {
					...
					if (parameter === 0) {
						...
						segment =
							curve.getPartLength(0, parameter) < curve.getPartLength(parameter, 1)
								? curve._segment1
								: curve._segment2;
					}
					this._segment = segment;
					...
					var c1 = this.getCurve(),
						...
						diff = abs(
							(c1.isLast() && c2.isFirst() ? -1 : c1.getIndex()) +
								this.getParameter() -
								((c2.isLast() && c1.isFirst() ? -1 : c2.getIndex()) + loc.getParameter())
						);
					res =
						(diff < 4e-7 ||
			...
			isCrossing: function() {
				...
				var t1 = this.getParameter(),
					...
					tMin = 4e-7,
					tMax = 1 - tMin;
				if ((t1 >= tMin && t1 <= tMax) || (t2 >= tMin && t2 <= tMax)) return !this.isTouching();
			...
			function insert(locations, loc, merge) {
				...
				function search(index, dir) {
					for (var i = index + dir; i >= -1 && i <= length; i += dir) {
						...
						if (!loc.getPoint().isClose(loc2.getPoint(), 2e-7)) break;
				...
				while (l <= r) {
					...
					var path1 = loc.getPath(),
						...
						diff =
							path1 === path2
								? loc.getIndex() + loc.getParameter() - (loc2.getIndex() + loc2.getParameter())
								: path1._id - path2._id;
					if (diff < 0) {
						r = m - 1;
					} else {
	...
	var PathItem = Item.extend({
		...
		getIntersections: function(path, include, _matrix, _returnFirst) {
			...
			if (!self && !this.getBounds(matrix1).touches(path.getBounds(matrix2))) return [];
		...
		setPathData: function(data) {
			var parts = data.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/gi),
				coords,
				relative = false,
				previous,
				control,
	...
	var Path = PathItem.extend(
		{
			...
			initialize: function Path(arg) {
				...
				var segments = Array.isArray(arg)
					? typeof arg[0] === 'object' ? arg : arguments
			...
			_hitTestSelf: function(point, options) {
				var that = this,
					...
					radius = hitStroke
						...
						: (hitFill && options.tolerance > 0) || hitCurves ? 0 : null;
				if (radius !== null) {
					if (radius > 0) {
				...
				smooth: function() {
					...
					for (var i = overlap; i <= n - overlap; i++) {
						...
						if (i < n) {
							...
							handleIn =
								i < n - 1
									? new Point(2 * knots[i + 1]._x - x[i + 1], 2 * knots[i + 1]._y - y[i + 1])
									: new Point((knots[n]._x + x[n - 1]) / 2, (knots[n]._y + y[n - 1]) / 2);
						}
					}
					if (closed && handleIn) {
			...
			statics: {
				...
				getStrokeBounds: function(segments, closed, style, matrix) {
					...
					function addRound(segment) {
						bounds = bounds.unite(
							joinBounds.setCenter(matrix ? matrix._transformPoint(segment._point) : segment._point)
						);
					}
					...
					function addJoin(segment, join) {
						...
						if (
							join === 'round' ||
							(!handleIn.isZero() && !handleOut.isZero() && handleIn.isCollinear(handleOut))
						) {
							addRound(segment);
						} else {
			...
			return {
				...
				Star: function() {
					...
					for (var i = 0; i < points; i++)
						segments[i] = new Segment(
							center.add(vector.rotate(step * i).multiply(i % 2 ? radius2 : radius1))
						);
					return createPath(segments, true, arguments);
				}
			};
	...
	var CompoundPath = PathItem.extend(
		{
			...
			reduce: function reduce() {
				...
				for (var i = children.length - 1; i >= 0; i--) {
					...
					if (path.isEmpty()) children.splice(i, 1);
				}
			...
			getPathData: function(_matrix, _precision) {
				...
				for (var i = 0, l = children.length; i < l; i++) {
					var child = children[i],
						mx = child._matrix;
					paths.push(
						child.getPathData(_matrix && !mx.isIdentity() ? _matrix.chain(mx) : _matrix, _precision)
					);
				}
				return paths.join(' ');
			}
	...
	PathItem.inject(
		new function() {
			...
			function finishBoolean(ctor, paths, path1, path2, reduce) {
				...
				if (reduce) result = result.reduce();
				result.insertAbove(
					path2 && path1.isSibling(path2) && path1.getIndex() < path2.getIndex() ? path2 : path1
				);
			...
			function computeBoolean(path1, path2, operation) {
				if (!path1._children && !path1._closed) return computeOpenBoolean(path1, path2, operation);
			...
			function divideLocations(locations) {
				...
				for (var i = locations.length - 1; i >= 0; i--) {
					...
					if (t < tMin) {
						...
						if (noHandles) clearSegments.push(segment);
					}
			...
			function getWinding(point, curves, horizontal, testContains) {
				...
				if (horizontal) {
					...
					if (yTop > -Infinity) windLeft = getWinding(new Point(px, yTop), curves, false, testContains);
					if (yBottom < Infinity) windRight = getWinding(new Point(px, yBottom), curves, false, testContains);
				} else {
					...
					for (var i = 0, l = curves.length; i < l; i++) {
						...
						if (
							winding &&
							((winding === 1 && py >= values[1] && py <= values[7]) ||
							...
							if (
								!(
									...
									(t < tMin && prevT > tMax && curve.previous === prevCurve)
								)
							) {
								var x = Curve.getPoint(values, t).x,
									...
									counted = false;
								if (
									(Numerical.isZero(slope) && !Curve.isStraight(values)) ||
									(t < tMin && slope * Curve.getTangent(curve.previous.values, 1).y < 0) ||
									(t > tMax && slope * Curve.getTangent(curve.next.values, 0).y < 0)
								) {
								...
								if (curve.previous !== curves[i - 1]) startCounted = t < tMin && counted;
							}
							prevCurve = curve;
							prevT = t;
						}
				...
				for (var i = 0; i < 3; i++) {
					...
					for (var k = 0, m = chain.length; k < m; k++) {
						...
						if (length <= curveLength) {
							if (length < epsilon || curveLength - length < epsilon) length = curveLength / 2;
			...
			function tracePaths(segments, operation) {
					...
					while (inter) {
						...
						if (
							...
							(!seg._visited &&
								...
								(!operator ||
									((!strict || isValid(seg)) &&
										((!(strict && nextInter && nextInter.isOverlap()) && isValid(nextSeg)) ||
											(!strict && nextInter && isValid(nextInter._segment))))))
						)
							return inter;
						inter = inter._next;
					}
				...
				for (var i = 0, l = segments.length; i < l; i++) {
					var seg = segments[i],
						path = null,
						finished = false;
					if (!isValid(seg, true)) continue;
					start = otherStart = null;
					while (!finished) {
						var inter = seg._intersection,
							handleIn = path && seg._handleIn;
						inter =
							(inter && (findBestIntersection(inter, true) || findBestIntersection(inter, false))) ||
							inter;
						var other = inter && inter._segment;
						if (other && isValid(other)) seg = other;
						if (seg._visited) {
			...
			return {
				...
				resolveCrossings: function() {
					...
					if (!crossings.length) return this;
		...
		getInteriorPoint: function() {
			...
			if (!this.contains(point)) {
				...
				for (var i = 0, l = curves.length; i < l; i++) {
					var values = curves[i].values;
					if (
						((curves[i].winding === 1 && y >= values[1] && y <= values[7]) ||
				...
				'hsl-rgb': function(h, s, l) {
					...
					for (var i = 0; i < 3; i++) {
						...
						c[i] =
							6 * t3 < 1
								? t1 + (t2 - t1) * 6 * t3
								: 2 * t3 < 1 ? t2 : 3 * t3 < 2 ? t1 + (t2 - t1) * (2 / 3 - t3) * 6 : t1;
					}
					return c;
				},
			...
			return Base.each(
				...
				},
				{
					...
					initialize: function Color(arg) {
						...
						if (!components) {
							values =
								argType === 'number' ? args : argType === 'object' && arg.length != null ? arg : null;
							if (values) {
								...
								if (arg.constructor === Color) {
									...
									type =
										'hue' in arg
											? 'lightness' in arg ? 'hsl' : 'hsb'
											: 'gradient' in arg || 'stops' in arg || 'radial' in arg
												? 'gradient'
												: 'gray' in arg ? 'gray' : 'rgb';
					...
					toCSS: function(hex) {
						...
						return hex
							? '#' +
								((1 << 24) + (components[0] << 16) + (components[1] << 8) + components[2])
									.toString(16)
									.slice(1)
	...
	var Style = Base.extend(
		...
		}(),
		{
			...
			equals: function(style) {
				return (
					style === this ||
					(style && this._class === style._class && Base.equals(this._values, style._values)) ||
					false
				);
			},
		...
		return {
			...
			isInView: function(el) {
				return (
					...
					DomElement.getViewportBounds(el).intersects(DomElement.getBounds(el, true))
				);
			},
	...
	var DomEvent = {
		...
		getPoint: function(event) {
			var pos = event.targetTouches
				? event.targetTouches.length ? event.targetTouches[0] : event.changedTouches[0]
				: event;
			return new Point(
	...
	DomEvent.requestAnimationFrame = new function() {
		...
		function handleCallbacks() {
			for (var i = callbacks.length - 1; i >= 0; i--) {
				...
				if (
					!el ||
					((PaperScope.getAttribute(el, 'keepalive') == 'true' || focused) && DomElement.isInView(el))
				) {
	...
	var View = Base.extend(
		Emitter,
		{
			...
			_events: Base.each(
				[ 'onResize', 'onMouseDown', 'onMouseUp', 'onMouseMove' ],
				function(name) {
					this[name] = {
						install: function(type) {
	...
	var CanvasView = View.extend(
		{
			...
			initialize: function CanvasView(project, canvas) {
				if (!(canvas instanceof HTMLCanvasElement)) {
					...
					if (size.isZero())
						throw new Error(
							'Cannot create CanvasView with the provided argument: ' + [].slice.call(arguments, 1)
						);
					...
					switch (type) {
						...
						case 'mouseup':
							...
							if (!stopped && item && item === downItem) {
								...
								callEvent(
									this,
									dblClick && downItem.responds('doubleclick') ? 'doubleclick' : 'click',
									event,
									downPoint,
									item
								);
				...
				if (name === 'command' && browser && browser.mac) {
					if (down) {
						...
						for (var code in commandFixMap) {
							if (code in charCodeMap) handleKey(false, code, commandFixMap[code], event);
						}
						commandFixMap = null;
					}
				}
	...
	var Tool = PaperScopeItem.extend({
		...
		_handleEvent: function(type, point, event) {
			...
			switch (type) {
				...
				case 'mouseup':
					if (
						!point.equals(this._point) &&
						this._updateEvent('mousedrag', point, this.minDistance, this.maxDistance, false, false, false)
					) {
				...
				case 'mousemove':
					while (
						this._updateEvent(type, point, this.minDistance, this.maxDistance, this._firstMove, true, false)
					) {
						called = this._fireEvent(type, event) || called;
		...
		function exportPath(item, options) {
			...
			if (matchShapes && !item.hasHandles()) {
				if (segments.length >= 3) {
					...
					var parts = [];
					for (var i = 0, l = segments.length; i < l; i++) parts.push(formatter.point(segments[i]._point));
		...
		function applyStyle(item, node, isRoot) {
			...
			Base.each(SVGStyles, function(entry) {
				var get = entry.get,
					...
					value = item[get]();
				if (
					entry.exportFilter ? entry.exportFilter(item, value) : !parent || !Base.equals(parent[get](), value)
				) {
					...
					attrs[entry.attribute] =
						value == null
							...
							: type === 'number'
								...
								: type === 'color'
									? value.gradient ? exportGradient(value, item) : value.toCSS(true)
									: type === 'array' ? value.join(',') : type === 'lookup' ? entry.toSVG[value] : value;
				}
			});
	...
	new function() {
			...
			return value === 'none'
				...
				: type === 'number'
					...
					: type === 'array'
						...
						: type === 'color' ? getDefinition(value) || value : type === 'lookup' ? lookup[value] : value;
		}
		...
		var importers = {
			...
			use: function(node) {
				...
				return definition
					? definition instanceof Symbol ? definition.place(point) : definition.clone().translate(point)
					: null;
			},
	...
	Base.exports.PaperScript = function() {
		...
		!(function(e, r) {
			...
			function c() {
				for (
					var e = br, r = fr.onComment && fr.locations && new a(), t = dr.charCodeAt((br += 2));
			...
			function u() {
				for (; pr > br; ) {
					...
					} else if (10 === e || 8232 === e || 8233 === e) ++br, fr.locations && (++Ar, (Sr = br));
					else if (e > 8 && 14 > e) ++br;
					else if (47 === e) {
			...
			function v(e) {
				...
				return r === e
					...
					: 33 == r && 60 == e && 45 == dr.charCodeAt(br + 2) && 45 == dr.charCodeAt(br + 3)
						? ((br += 4), c(), u(), g())
			...
			function w(e, r) {
				for (var t = br, n = 0, a = 0, o = null == r ? 1 / 0 : r; o > a; ++a) {
					var i,
						s = dr.charCodeAt(br);
					if (
						((i = s >= 97 ? s - 97 + 10 : s >= 65 ? s - 65 + 10 : s >= 48 && 57 >= s ? s - 48 : 1 / 0),
						i >= e)
					)
						break;
					++br, (n = n * e + i);
			...
			function E(e) {
				...
				return (
					n
						? (s = parseFloat(c))
						: a && 1 !== c.length
							? /[89]/.test(c) || Tr ? t(r, 'Invalid number') : (s = parseInt(c, 8))
							: (s = parseInt(c, 10)),
					i(Or, s)
				);
			}
			function A(e) {
				...
				for (var r = ''; ; ) {
					...
					if (92 === n) {
						...
						(13 === n || 10 === n || 8232 === n || 8233 === n) && t(yr, 'Unterminated string constant'),
							(r += String.fromCharCode(n)),
							++br;
				}
			}
			...
			function I() {
				...
				for (var e, r = !0, n = br; ; ) {
					...
					if (Yt(a)) Bt && (e += dr.charAt(br)), ++br;
						...
						Bt || (e = dr.slice(n, br)),
							(Bt = !0),
							117 != dr.charCodeAt(++br) && t(br, 'Expecting Unicode escape sequence \\uXXXX'),
							++br;
						var o = S(4),
			...
			function L() {
				...
				return (
					Bt ||
						(Wt(e)
							? (r = lt[e])
							: ((fr.forbidReserved && (3 === fr.ecmaVersion ? Mt : zt)(e)) || (Tr && Xt(e))) &&
				...
				switch (e) {
						...
						for (var s, c; wr != ht; )
							if (wr === zr || wr === Jr) {
								...
								s && j(s, 'SwitchCase'),
									...
									u
										? (s.test = K())
										: (c && t(Ir, 'Multiple default clauses'), (c = !0), (s.test = null)),
									z(gt);
					...
					case et:
						...
						return (
							...
							r.handler || r.finalizer || t(r.start, 'Missing catch or finally clause'),
							j(r, 'TryStatement')
						);
					case rt:
			...
			function G(e, r) {
				for (e.declarations = [], e.kind = 'var'; ; ) {
					...
					if (
						((n.id = lr()),
						Tr && Nt(n.id.name) && t(n.id.start, 'Binding ' + n.id.name + ' in strict mode'),
			...
			function nr(e, r) {
				...
				if (F(ft)) {
					var t = O(e);
					return (
						(t.object = e), (t.property = K()), (t.computed = !0), z(dt), nr(j(t, 'MemberExpression'), r)
					);
				}
				if (!r && F(mt)) {
					var t = O(e);
			...
			function cr(e, r) {
				...
				if (
					...
					for (var i = e.id ? -1 : 0; i < e.params.length; ++i) {
						...
						if (
							((Xt(s.name) || Nt(s.name)) && t(s.start, "Defining '" + s.name + "' in strict mode"),
							i >= 0)
						)
							for (var c = 0; i > c; ++c)
			...
			function lr(e) {
				var r = q();
				return (
					(r.name = wr === Fr ? Cr : (e && !fr.forbidReserved && wr.keyword) || X()),
					(Er = !1),
					U(),
					j(r, 'Identifier')
				);
			...
			var Bt,
				...
				Qt = (e.isIdentifierStart = function(e) {
					return 65 > e
						? 36 === e
						: 91 > e ? !0 : 97 > e ? 95 === e : 123 > e ? !0 : e >= 170 && $t.test(String.fromCharCode(e));
				}),
					...
					return 48 > e
						...
						: 58 > e
							...
							: 65 > e
								...
								: 91 > e
									? !0
									: 97 > e ? 95 === e : 123 > e ? !0 : e >= 170 && _t.test(String.fromCharCode(e));
				}),
				Zt = { kind: 'loop' },
				en = { kind: 'switch' };
		});
					...
					case 'AssignmentExpression':
						...
						if (
							...
							if (node.type === 'UpdateExpression') {
								...
								if (
									!node.prefix &&
									(parentType === 'AssignmentExpression' || parentType === 'VariableDeclarator')
								) {
								...
								if (/^.=$/.test(node.operator) && node.left.type !== 'Literal') {
									...
									replaceCode(
										node,
										left + ' = __$__(' + left + ', "' + node.operator[0] + '", ' + right + ')'
									);
								}
							}
						}
		...
		function execute(code, scope, url, options) {
			...
			if (browser.chrome || browser.firefox) {
				...
				script.appendChild(
					document.createTextNode('paper._execute = function(' + params + ') {' + code + '\n}')
				);
